...
@check("guest endpoints are deliberate and state-validated")
def check_guest_endpoints():
    """
    Every `allow_guest=True` method is reachable by anyone on the internet.
    This does not forbid them — enrolment, ingest and the consent callback all
    need to be — it asserts the list is a known list, so a new one has to be
    added here consciously rather than appearing in a diff nobody read.
    """
    expected = {
        ("api/ingest.py", "enrol"),
        ("api/ingest.py", "upload_batch"),
        ("api/ingest.py", "heartbeat"),
        ("api/session.py", "describe"),
        ("api/session.py", "consent"),
        ("oauth.py", "callback"),
        ("oauth.py", "status"),
    }
    found = set()
    ...
    for item in sorted(found - expected):
        fail(f"undeclared guest endpoint: {item[0]}::{item[1]} — add it to verify_tree "
             f"once you are satisfied it is safe to expose")
    for item in sorted(expected - found):
        fail(f"declared guest endpoint no longer exists: {item[0]}::{item[1]}")


@check("the consent callback validates its inputs")
def check_consent_callback_hardening():
    """
    The callback is a guest-reachable browser redirect carrying a tenant id
    that gets interpolated into a token endpoint URL. Three properties are
    non-negotiable.
    """
    path = os.path.join(APP, "oauth.py")
    if not os.path.exists(path):
        return
    src = open(path, encoding="utf-8").read()

    if "hmac.compare_digest" not in src:
        fail("oauth.py: state signature must be compared in constant time")
    if "GUID.match" not in src:
        fail("oauth.py: the tenant id must be validated as a GUID before use in a URL")
    if "STATE_TTL_SECONDS" not in src:
        fail("oauth.py: the state token must expire")


@check("www routes resolve to a template and a controller")
def check_www_routes():
    """
    A redirect target that does not exist as a page renders a 404 to the
    customer at the exact moment they have just authorised access — the worst
    ...
    for name in os.listdir(www):
        if not name.endswith(".html"):
            continue
        stem = name[:-5]
        if not os.path.exists(os.path.join(www, f"{stem}.py")):
            fail(f"www/{name} has no {stem}.py controller")
        html = open(os.path.join(www, name), encoding="utf-8").read()
        if "{% raw %}" in html:
            fail(f"www/{name} contains a raw block, which renders the template literally")

    for path in _py_files():
        src = open(path, encoding="utf-8").read()
        for route in re.findall(r'get_url\(\)\}/([a-z0-9\-]+)', src):
            if not os.path.exists(os.path.join(www, f"{route}.html")):
                fail(f"{os.path.relpath(path, ROOT)} redirects to /{route}, which has no page")
...
CHECKS = [
    ...
    check_guest_endpoints,
    check_consent_callback_hardening,
    check_www_routes,
]
