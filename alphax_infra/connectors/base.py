...
DEFAULT_TIMEOUT = 30
MAX_PAGES = 200
USER_AGENT = "AlphaX-Infra/0.2.0 (+https://neotec.sa)"
...
def credentials(connector_name: str) -> dict:
    ...
    if (doc.get("auth_mode") or "") == "Admin Consent":
        # The customer never issued us a credential. An app-only token for a
        # consented tenant is obtained with OUR application's client_id and
        # secret against THEIR tenant's token endpoint — which is exactly the
        # shape oauth_token already takes, so no connector changes downstream.
        cfg = frappe.get_cached_doc("AlphaX Infra Settings")
        return {
