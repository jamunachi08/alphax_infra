...
**The fast path (recommended).** Register one multi-tenant application in your
own Entra tenant, once — see `SETUP_ADMIN_CONSENT.md`. Then every engagement is:
create a client, create an assessment session, send the `/infra-onboarding` link.
The customer's Global Administrator signs in, accepts Microsoft's own read-only
permission screen, and discovery starts. No credential is created in their
tenant and nothing is sent to you.

**The manual path** is still supported for customers who insist on issuing their
own app registration. Order matters, and the app enforces it:
